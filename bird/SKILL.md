...
name: bird
description: >
  X/Twitter CLI for reading, searching, posting, and engagement via cookie auth.
  Use this skill when the user wants to interact with X (Twitter) — reading tweets,
  searching posts, browsing timelines, posting, or engaging with content.
  Requires the bird CLI tool (@steipete/bird) to be installed.
---

<!--
  Attribution:
  This skill is based on the bird skill from openclaw/openclaw (MIT License).
  bird CLI is created by Peter Steinberger (@steipete).
  Homepage: https://bird.fast
  CLI package: https://www.npmjs.com/package/@steipete/bird
  Original: https://github.com/openclaw/openclaw
-->

# bird

Fast X/Twitter CLI using GraphQL + cookie auth, created by [Peter Steinberger](https://github.com/steipete).

- CLI: [@steipete/bird](https://www.npmjs.com/package/@steipete/bird)
- Homepage: [bird.fast](https://bird.fast/)

## Authentication

`bird` uses cookie-based auth. Authentication priority:

1. **Environment variables** (recommended): Set `AUTH_TOKEN` and `CT0` in `~/.bashrc`
2. **CLI args**: `--auth-token` / `--ct0`
3. **Browser cookies**: `--cookie-source=chrome|firefox|safari`
